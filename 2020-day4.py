...
if __name__ == "__main__":
    ...
    for line in data:
        if (len(line.keys())==8) or (len(line.keys())==7 and 'cid' not in line.keys()):
            valid+=1
    ...
    for line in data:
        if (len(line.keys())==8) or (len(line.keys())==7 and 'cid' not in line.keys()):
            if len(line['byr']) != 4 or int(line['byr'])<1920 or int(line['byr'])>2002:
                continue
            if len(line['iyr']) != 4 or int(line['iyr'])<2010 or int(line['iyr'])>2020:
                continue

            if len(line['eyr'])!= 4 or int(line['eyr']) < 2020 or int(line['eyr']) > 2030:
                continue
